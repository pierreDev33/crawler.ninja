/**
 * This is a fork of https://github.com/sylvinus/node-crawler
 *
 * in order to add more features :
 *  - response time
 *  - a better proxy support
 *  - a persistence queue (not yet implemented)
 *
 */
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    if (options.proxyList) {
      ropts.proxy  = options.proxyList.getProxy().getUrl();
      options.proxy = ropts.proxy;
    }
    ...
    var req = request(_.pick.apply(this,[ropts].concat(requestArgs)), function(error,response) {
        ...
        if (ropts.proxy) {
          console.log(ropts.proxy);
        }
        response.proxy = ropts.proxy;
