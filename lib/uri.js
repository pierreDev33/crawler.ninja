 ...
 *
 * @TODO : avoid the try & catch blocks
 *
 */
...
var linkToURI = function(pageUri, link) {
    ...
    try {
      ...
      var data = {errorCode : "URIError", uri : link, error : true,
                  message : "uri.js : Impossible to build an standard URI for link " + link + " in page " + pageUri};
      log.error(data);
      return "";
    }
...
var isRelativeOrAbsolute = function (uri) {
    ...
    try {
        ...
        var data = {errorCode : "URIError", uri : link, error : true,
                    message : "uri.js : Impossible to check if isRelativeOrAbsolute " + uri };
        log.error(data);
        return false;
    }
...
var buildURI = function (uri) {
  ...
  try {
    ...
    var data = {errorCode : "URIError", uri : uri,error : true, message : "uri.js : Impossible to build URI for " + uri}
    log.error(data);
    return URI("");
  }
