 ...
 * The request queue
 *
 * its main job is to make the http requests & get the responses
  ...
  this.q = async.queue(
               function (options, callback) {
                    ...
                    if (options.domainBlackList.indexOf(URI.domainName(options.url)) > 0) {
                        options.onCrawl({code:"DOMAINBLACKLIST"}, options);
                        callback();
                        return;
                    }

                    // Case of too many errors on a host
                    // => use a crawl rate limit or stop to crawl this domain
                    var host = URI.host(options.url);
                    if (self.hostErrors.has(host)) {
                        var errorInfo = self.hostErrors.get(host);
...
Requester.prototype.execHttp = function (options, callback) {
    ...
    request(options, function(error, result) {
      ...
      if (error) {
        self.onRequestError(error, options, result);
      }
      else {
        options.onCrawl(null, result);
      }
      callback();
...
Requester.prototype.onRequestError = function (error, options, result) {
  if (error.code ==  'ETIMEDOUT' || error.code ==  'ESOCKETTIMEDOUT') {

    // If too many timedouts, decrease the crawl rate
    // & make another request on the same url
    if (this.hasTooManyErrors(result)) {
        result = this.decreaseCrawlRate(result);

        this.history.remove(result.url);
        this.queue(result);
        return;
    }
...
Requester.prototype.queue = function (options) {
    ...
    log("Add to the queue : " + options.url, options); 
...
Requester.prototype.hasTooManyErrors = function (options) {
    ...
    if (this.hostErrors.has(host)) {
        errorInfo = this.hostErrors.get(host);
    }
    else {
...
Requester.prototype.decreaseCrawlRate = function(options) {
    ...
    var host = URI.host(options.url);
    var errorInfo = this.hostErrors.get(host);
