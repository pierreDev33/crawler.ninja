...
(function () {
   ...
   *
   */
  function init (maxConnections, crawlCallback, endCallback, proxies) {
      createQueue(maxConnections, crawlCallback, endCallback);
      proxyList = proxies;
  }
   ...
   *
   */
  function createQueue (maxConnections, crawlCallback, endCallback) {

    //TODO : requestQueue = async.queue(onUrlToCrawl,maxConnections);
  ...
  function onStoreError(error, options, callback) {
      ...
      requestQueue.onCrawl({code:"STOPCRAWL"}, options, function(error){
        process.nextTick(function() {callback(error)});
      });
  }
  ...
  function crawlWithErrors(options, errorInfo, callback) {
    ...
    if (errorInfo.stopCrawlOnThisHost) {
  ...
  function onRequestError(error, options, result, callback) {
      ...
      // 1. Check the crawl rate and if necessary decrease it for slower crawl
      ...
      // if it is a connection error, recrawl the url if the maximum of retries is not yet reached
      ...
      requestQueue.onCrawl(error, result, function(error) {
        process.nextTick(function() {callback(error)});
      });
  ...
  /**
   * In the case of a timeout error, this method is called in order to check
  ...
  function decreaseCrawlRate(params, callback) {
      ...
      // we stop to crawl on this domain if all rate limits have been used
        ...
        params.errorInfo.stopCrawlOnThisHost = true;
      }
  ...
  function recrawlUrl(params, callback) {
    ...
    if (params.result.maxRetries > 1) {
        ...
        log.warn({"url" : params.options.url, "step" : "queue-requester.recrawlUrl", "message" : "Recrawl -  retries left : " + params.result.maxRetries});
        ...
        requestQueue.onCrawl(params.error, params.result, function(error){
